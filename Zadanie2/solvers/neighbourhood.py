...
class Neighbourhood(ABC):
    ...
    def get_moves_of_type(self, move_type, i=0, j=0, step=1):
        if move_type == MoveType.NODE_SWAP_IN_A:
            ...
            return self.get_candidate_moves_in_cycle(self.cycleA, i, move_type, n=10)
        elif move_type == MoveType.CANDIDATE_IN_B:
            return self.get_candidate_moves_in_cycle(self.cycleB, i, move_type, n=10)
        else:
            return []
        ...
        for i in range(s1, np.sign(step) * (s1 + len(self.cycleA)), step):
            for j in range(s2, (s2 + len(self.cycleB)) * np.sign(step), step):
                if i != j:
                    ...
                    if real_i > buddy_i or (real_i == 0 and buddy_i == last_i):
                        if not (real_i == last_i and buddy_i == 0):
                    ...
                    if real_j > buddy_j or (real_j == 0 and buddy_j == last_i):
                        if not (real_j == last_i and buddy_j == 0):
